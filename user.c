...
BOOL	g_ack_enable;
int ActivePorts = 0;
unsigned short HBReload = 75; // ~10khz interrupt
...
typedef struct tag_IRSTRUCT
{
	unsigned int IRVal:1;
	unsigned char IRCount;
	unsigned char IRBits[64/8];
	unsigned char IRBit;
	volatile unsigned char IRReady;
...
{	
	...
	if (PIR1bits.TMR2IF)
	{
		...
		if (kPRIMED == g_RC_state[g_RC_primed_ptr])
		{
			...
			// And set this Min's state to timing
...
//void high_ISR(void)
{
	...
	if (PIR2bits.TMR3IF)
	{
		...
		// either HBridge OR Frequency active
		if (ActivePorts)
		{
			// reload
			TMR3H = HBReload >> 8;
...
{   
	...
	for (i = 0; i < 3; i++)
	{
		if (IR[i].IRReady)
		{
			...
			for (j = 0; j < (IR[i].IRBit-1+7)/8; j++)
				{
				printf( "%02.2x", IR[i].IRBits[j]);
				}

			printf( "\r\n" );
			memset( IR[i].IRBits, 0, sizeof(IR[i].IRBits) );
			IR[i].IRReady = 0;
			IR[i].IRBit = 0;
		}
	}
...
// PWM1 on port C1
// PWM2 on port C2
// <PWM1level> and <PWM2level> are between 0 and 100
void parse_W_packet (void)
{
	unsigned char PWM1cycle;
	unsigned int PWM2cycle;

	extract_number (kUCHAR, &PWM1cycle, kREQUIRED);
	extract_number (kUINT, &PWM2cycle, kREQUIRED);
...
//		TRISCbits.TRISC1=0;
		...
		//79 = (48000/38 - (4xPS))/4xPS;
		OpenEPWM1( 79, ECCP_1_SEL_TMR36 );	//PWM period =[(period ) + 1] x 4 x TOSC x TMR2 prescaler 
		SetOutputEPWM1( SINGLE_OUT, PWM_MODE_1 );
		SetDCEPWM1( PWM1cycle );

		// enable write to PPS
		PPSCON &= ~1;
		// assign ECCP1 output A to RP3
		RPOR3 = 14;
		
		// enable output on RB0
		TRISB &= ~1;
	...
	{
//		TRISCbits.TRISC2=0; 
		//OpenPWM2( 159 );	//PWM period =[(period ) + 1] x 4 x TOSC x TMR2 prescaler 
		//SetDCPWM2( PWM2cycle );
	}else{
		//ClosePWM2();
	}
...
#define MAX_HBRIDGES 4

unsigned char portdir[MAX_HBRIDGES] = {0};
unsigned char pindir[MAX_HBRIDGES] = {0};
unsigned char portduty[MAX_HBRIDGES] = {0};
unsigned char pinduty[MAX_HBRIDGES] = {0};
unsigned char valdir[MAX_HBRIDGES] = {0};
unsigned char valduty[MAX_HBRIDGES] = {0};

int HBCount = 0;
unsigned char HBCycles = 0;




void ProcessRC5( IRSTRUCT *IR, unsigned char val )
{
	// if we have a transition
	if (IR->IRVal != val)
	{
		unsigned char islong = 0;
		// synchronise RunningCount
		if (IR->IRCount > 5)
			islong = 1;

		if (0 == val)
		{
			unsigned char Bit = IR->IRBit - 1;
			if (islong)
...
void ProcessIR( IRSTRUCT *IR, unsigned char val )
{
	...
	else
	{
		...
		{
			// measure start pulse to determine which type.
			// 9ms -> RC5 
			if (Bit == 0)
			{
				...
				if ((IR->IRCount > 25) && (IR->IRCount < 36))
				{
					IR->Type = RECS80SLOW;
				}

				if ((IR->IRCount > 13) && (IR->IRCount < 21))
				{
					IR->Type = RECS80FAST;
				}
	...
	if (HBCount >= 10)
	{
		...
		for (i = 0; i < ActivePorts; i++)
		{
			if (valdir[i])
			{
				...
				{
				case 'A':
					bitset (LATA, pindir[i]);
					break;
				case 'B':
					bitset (LATB, pindir[i]);
					break;
				case 'C':
					bitset (LATC, pindir[i]);
					break;
				case 'D':
					bitset (LATD, pindir[i]);
					break;
				}
			}
			else
			{
				...
				{
				case 'A':
					bitclr (LATA, pindir[i]);
					break;
				case 'B':
					bitclr (LATB, pindir[i]);
					break;
				case 'C':
					bitclr (LATC, pindir[i]);
					break;
				case 'D':
					bitclr (LATD, pindir[i]);
					break;
				}
			}
			...
			if (valduty[i] > HBCycles)
			{
				...
				{
				case 'A':
					bitset (LATA, pinduty[i]);
					break;
				case 'B':
					bitset (LATB, pinduty[i]);
					break;
				case 'C':
					bitset (LATC, pinduty[i]);
					break;
				case 'D':
					bitset (LATD, pinduty[i]);
					break;
				}
			}
			else
			{
				...
				{
				case 'A':
					bitclr (LATA, pinduty[i]);
					break;
				case 'B':
					bitclr (LATB, pinduty[i]);
					break;
				case 'C':
					bitclr (LATC, pinduty[i]);
					break;
				case 'D':
					bitclr (LATD, pinduty[i]);
					break;
				}
			}
		}
...
// HS is for HBridge pin setup
// expects HB,<portdir1>,<pindir1>,<portduty1>,<pinduty1>
// there may be up to 4 pairs of port pins
void parse_HS_packet(void)
{
	int i;
	volatile near unsigned char Dummy;
	memset(portdir, 0, sizeof(portdir));
	memset(pindir, 0, sizeof(pindir));
	memset(portduty, 0, sizeof(portduty));
	memset(pinduty, 0, sizeof(pinduty));
	ActivePorts = 0;
	
	for (i = 0; i < MAX_HBRIDGES; i++)
	{
		extract_number (kUCASE_ASCII_CHAR, &portdir[i], kOPTIONAL);
		extract_number (kUCHAR, &pindir[i], kOPTIONAL);
	
		extract_number (kUCASE_ASCII_CHAR, &portduty[i], kOPTIONAL);
		extract_number (kUCHAR, &pinduty[i], kOPTIONAL);
		if (portduty[i] != 0)
		{
			volatile near unsigned char *TRIS;
			volatile near unsigned char *LAT;
			ActivePorts++;
			TRIS = &Dummy;
			switch (portdir[i])
			{
			case 'A':	TRIS = &TRISA; LAT = &LATA;	break;
			case 'B':	TRIS = &TRISB; LAT = &LATB;	break;
			case 'C':	TRIS = &TRISC; LAT = &LATC;	break;
			case 'D':	TRIS = &TRISD; LAT = &LATD;	break;
			}
			*TRIS = *TRIS & ~(1 << pindir[i]);
			*LAT  = *LAT  & ~(1 << pindir[i]);
			...
			*TRIS = *TRIS & ~(1 << pinduty[i]);
			*LAT  = *LAT  & ~(1 << pinduty[i]);
		}
	}

	printf ((far rom char *)"set %d HB ports\r\n", ActivePorts);

	for (i = 0; i < ActivePorts; i++)
	{
		printf((far rom char *)"%c%d-%c%d\r\n", portdir[i], pindir[i], portduty[i], pinduty[i]);
	}
...
// HB is for HBridge control (PWM + Dir)
// runs 200hz pwm on 2 pins, and dir on two pins
// "HB,<freq>,<port>,<pin>,<duty_cycle><CR>"
// <port> is "A", "B", "C" and indicates the port ("D" and "E" for 18F4550s)
// <pin> is a number between 0 and 7 and indicates which pin used for output
// <duty_cycle> is optional, and is from 1 through 99 and indicates the percentage of the 
//   <freq> period that the pin should be high (effectively PWM)

void parse_HB_packet(void)
{
	int i;
	char val[MAX_HBRIDGES];
	

	for (i = 0; i < MAX_HBRIDGES; i++)
	{
		if (kEXTRACT_OK == extract_number (kCHAR, &val[i], kOPTIONAL))
		{
			if (val[i] < 0)
			{
				valdir[i] = 1;	
